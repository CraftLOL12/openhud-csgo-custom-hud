...
import { createMatch, deleteMatch, getCurrentMatch, getMatchById, getMatches, setCurrentMatch, updateCurrentMatch, updateMatch, getTeamById, } from "../services/index.js";
import { io } from "../sockets/socket.js";
import { GSI } from "../sockets/GSI.js";
// Controllers do know they are working with express.
export const getMatchesController = async (_req, res) => {
    try {
...
export const getCurrentMatchController = async (_req, res) => {
    try {
        ...
        if (match.left.id) {
            getTeamById(match.left.id).then((left) => {
                const gsiTeamData = {
                    id: left._id,
                    name: left.name,
        ...
        if (match.right.id) {
            getTeamById(match.right.id).then((right) => {
                const gsiTeamData = {
                    id: right._id,
                    name: right.name,
...
export const updateCurrentMatchController = async (req, res) => {
    const match = req.body;
    try {
...
export const setCurrentMatchController = async (req, res) => {
    const id = req.params.id;
