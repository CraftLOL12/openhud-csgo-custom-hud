...
import { getCoaches, getCurrentMatch, updateCurrentMatch, } from "../services/index.js";
...
export const readGameData = async (req, res) => {
    try {
        const data = req.body;
        ...
        const coaches = (await getCoaches()).map((coach) => coach.steamid);
...
GSI.on("intermissionEnd", async () => {
    ...
    if (match) {
        const { vetos } = match;
        const updatedVetos = vetos.map((veto) => veto.mapName === GSI.current?.map.name
